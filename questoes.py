...
if menu == "Início":
    ...
    st.write("Selecione 'Simulado ANCORD' ao lado para configurar seus estudos por módulo.")
...
# --- PÁGINA: SIMULADO ---
    ...
    if not st.session_state.simulado_iniciado:
        # Extrai os módulos únicos do seu novo BANCO_QUESTOES
        ...
        qtd = st.slider("Quantidade de questões:", 5, len(BANCO_QUESTOES), 10)
        ...
        if st.button("⬅️ Trocar Módulos/Configuração"):
            st.session_state.simulado_iniciado = False
            st.rerun()
        ...
        with st.form("form_simulado"):
            for idx, q in enumerate(st.session_state.questoes_sorteadas):
                ...
                # Transforma o dicionário de opções em uma lista de strings: "A) texto"
                lista_opcoes = [f"{k}) {v}" for k, v in q['opcoes'].items()]
            ...
            if st.form_submit_button("Finalizar e Salvar"):
                ...
                for idx, q in enumerate(st.session_state.questoes_sorteadas):
                    ...
                    if resp:
                        # Pega apenas a primeira letra da resposta escolhida (ex: "A")
